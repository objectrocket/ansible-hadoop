...
DOCUMENTATION = '''
---
module: cloudera_deploy_impala
short_description: deploy impala / delete a Cloudera cluster
description:
     - deploy impala / deletes a Cloudera cluster using Cloudera Manager.
version_added: "2.1"
options:
  name:
    description:
...
def build_impala_config(HDFS_SERVICE_NAME, HBASE_SERVICE_NAME, HIVE_SERVICE_NAME, CLUSTER_HOSTS)
    ...
    return (IMPALA_SERVICE_NAME, IMPALA_SERVICE_CONFIG, IMPALA_SS_HOST, IMPALA_SS_CONFIG, IMPALA_CS_HOST, IMPALA_CS_CONFIG, IMPALA_ID_HOSTS, IMPALA_ID_CONFIG)


  def deploy_impala(cluster, impala_service_name, impala_service_config, impala_ss_host, impala_ss_config,
                    impala_cs_host, impala_cs_config, impala_id_hosts, impala_id_config):
...
def main():
    ...
    build_impala_config(hdfs_service_name, hbase_service_name, hive_service_name, cluster_hosts)
    ...
    if state == "absent":
        ...
        try:
            ...
            module.fail_json(msg='Failed to deploy impala.\nError is %s' % e)
